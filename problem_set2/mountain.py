...
class Mountain:
    def __init__(self):
        ...
        self.actions_to_dir = {"upforward": (-1, 1), "forward": (0, 1),
                               "downforward": (1, 1)}  # in terms of (i,j)/(y,x)
    ...
    def next_state(self, state: Tuple[int, int], action: str):
        ...
        if action == "forward":
            if state[1] <= 98:
                ...
                return state
        if action == "upforward":
            if state[0] > 0 and state[1] <= 98:
                return state[0] - 1, state[1] + 1
            elif state[0] == 0 and state[1] <= 98:
                return state[0], state[1] + 1
            else:
                return state
        if action == "downforward":
            if state[0] < 30 and state[1] <= 98:
                return state[0] + 1, state[1] + 1
            elif state[0] == 30 and state[1] <= 98:
                return state[0], state[1] + 1
            else:
                return state
    ...
    def get_time(self, state: Tuple[int, int]) -> float:
        ...
        of distance on the hill depends directly on how rough roughness is equivalent to the time.
