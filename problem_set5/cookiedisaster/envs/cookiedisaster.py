...
class CookieDisasterEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        for i in np.linspace(0.1, 9.9, 20):
            self.cookie_positions.append(((min(np.random.normal(i, 0.02), 9.8) * self.scale_x) * 0.95 - 20 / 2,
                                          np.random.normal(self.window_size[1] / 1.8, 0.1) - 20 / 2))
            self.cookie_positions.append(((min(np.random.normal(i, 0.01), 9.8) * self.scale_x - 20 / 2,
