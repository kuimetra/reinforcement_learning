...
class CookieDisasterEnv(gym.Env):
    ...
    def __init__(self, config=config_v1, render_mode=None):
        ...
        for i in np.linspace(0.1, self.width - 0.1, 20):
            self.cookie_positions.append(((min(np.random.normal(i, 0.02),
                                               self.width - 0.2) * self.scale_x) * 0.95 - 20 / 2,
                                          np.random.normal(self.window_size[1] / 1.8, 0.1) - 20 / 2))
            self.cookie_positions.append(((min(np.random.normal(i, 0.01), self.width - 0.2) * self.scale_x - 20 / 2,
                                           np.random.normal(self.window_size[1] / 2, 0.1) - 20 / 2)))
            self.cookie_positions.append(((
                (min(np.random.normal(i, 0.02), self.width - 0.2) * self.scale_x * 0.95) - 20 / 2,
                np.random.normal(self.window_size[1] / 2.2, 0.1) - 20 / 2)))
